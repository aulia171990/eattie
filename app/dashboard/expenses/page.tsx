...
import { ExpenseApprovalButtons } from '@/components/shared/expense-approval-buttons'
          ...
          <table className="w-full">
            <thead>
              <tr style={{ background: 'hsl(var(--surface-raised))' }}>
                {['Tanggal', 'Kategori', 'Deskripsi', 'Jumlah', 'Status', 'Dicatat Oleh', 'Aksi'].map((h) => (
                  ...
                  <tr key={exp.id} className="border-t hover:bg-gray-50/50" style={{ borderColor: 'hsl(var(--border))' }}>
                    ...
                    <td className="px-4 py-3">
                      {(() => {
                        const statusMap: Record<string, { label: string; bg: string; fg: string }> = {
                          pending: { label: 'Menunggu', bg: 'hsl(var(--warning) / 0.1)', fg: 'hsl(var(--warning))' },
                          approved: { label: 'Disetujui', bg: 'hsl(var(--success) / 0.1)', fg: 'hsl(var(--success))' },
                          rejected: { label: 'Ditolak', bg: 'hsl(var(--danger) / 0.1)', fg: 'hsl(var(--danger))' },
                        }
                        const s = statusMap[exp.status as string] || statusMap.pending
                        return (
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <ExpenseApprovalButtons expenseId={exp.id} status={exp.status as string} />
