import { getOpname, submitOpname, approveOpname } from '@/actions/stock-opname'
...
function ApproveOpnameForm({ action }: { action: unknown }) {
  return (
...
export default async function OpnameDetailPage({
  ...
  const boundApprove = approveOpname.bind(null, id)
  const canEdit = opname.status === 'in_progress' || opname.status === 'draft'
  const canApprove = !opname.approved_by
      ...
      <PageHeader
        ...
        action={
          <div className="flex gap-2 items-center">
            <StatusBadge status={opname.status ?? ''} type="purchase" />
            {canApprove && !canEdit && <ApproveOpnameForm action={boundApprove} />}
          </div>
        }
      />
