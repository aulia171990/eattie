  ...
  return (
    <div className="p-6 max-w-lg">
      ...
      <div className="bg-white rounded-xl border p-6" style={{ borderColor: 'hsl(var(--border))' }}>
        <p className="text-sm mb-4" style={{ color: 'hsl(var(--text-muted))' }}>Sistem akan membuat sesi opname baru dengan semua bahan baku aktif.</p>
        <form action={createOpnameFromForm}>
          <button type="submit" className="w-full py-2.5 px-4 rounded-lg text-sm font-medium text-white" style={{ background: 'hsl(var(--primary))' }}>Mulai Stock Opname</button>
        </form>
      </div>
    </div>
  )
