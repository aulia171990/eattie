...
export default async function InventoryPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const totalValue = ingredients.reduce((sum, i) => sum + (i.current_stock * (i.price_per_unit ?? 0)), 0)
  // ingredients.min_stock / price_per_unit are schema-nullable (no NULL rows currently in DB); guard with ?? 0
  const lowStockItems = ingredients.filter(i => i.current_stock <= (i.min_stock ?? 0))
            ...
            <table className="w-full">
              ...
              <tbody>
                {ingredients.map(item => {
                  const stockStatus = item.current_stock <= 0 ? 'out' : item.current_stock <= (item.min_stock ?? 0) ? 'critical' : item.reorder_point && item.current_stock <= item.reorder_point ? 'low' : 'normal'
                  return (
                    <tr key={item.id} className="border-t hover:bg-gray-50/50 transition-colors" style={{ borderColor: 'hsl(var(--border))' }}>
                      ...
                      <td className="px-4 py-3">
                        ...
                        {(item.min_stock ?? 0) > 0 && <span className="text-xs ml-1" style={{ color: 'hsl(var(--text-muted))' }}>/ min {formatNumber(item.min_stock ?? 0)}</span>}
                      </td>
                      <td className="px-4 py-3 text-xs" style={{ color: 'hsl(var(--text-muted))' }}>{item.base_unit}</td>
                      <td className="px-4 py-3 text-sm" style={{ color: 'hsl(var(--foreground))' }}>{formatCurrency(item.price_per_unit ?? 0)}</td>
