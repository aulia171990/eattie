import { getPurchase, approveReceive } from '@/actions/stock-purchases'
...
function ApprovePurchaseForm({ action }: { action: unknown }) {
  return (
...
export default async function PurchaseDetailPage({
  ...
  const boundApprove = approveReceive.bind(null, id)
  const canApprove = po.status === 'ordered' && !po.approved_by
      ...
      <PageHeader
        ...
        action={
          <div className="flex gap-2 items-center">
            ...
            {canApprove && <ApprovePurchaseForm action={boundApprove} />}
            {po.status === 'ordered' && (
              <Link
