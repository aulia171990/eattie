...
export default async function IngredientDetailPage({
  ...
  const stockStatus =
    ingredient.current_stock <= 0
      ? 'out'
      : ingredient.current_stock <= (ingredient.min_stock ?? 0)
      ? 'critical'
  ...
  const infoRows = [
    ...
    { label: `Harga/${ingredient.base_unit || 'Unit'}`, value: formatCurrency(ingredient.price_per_unit ?? 0) },
    ...(ingredient.purchase_unit && ingredient.conversion_rate
      ? [{ label: `Harga/${ingredient.purchase_unit}`, value: formatCurrency((ingredient.price_per_unit ?? 0) * (ingredient.conversion_rate ?? 1)) }]
      : []),
            ...
            <div className="mt-3 pt-3 border-t space-y-1" style={{ borderColor: 'hsl(var(--border))' }}>
              <div className="flex justify-between text-xs">
                ...
                <span style={{ color: 'hsl(var(--text-secondary))' }}>
                  {formatNumber(ingredient.min_stock ?? 0)} {ingredient.base_unit}
                </span>
              </div>
              {ingredient.reorder_point != null && (
                <div className="flex justify-between text-xs">
