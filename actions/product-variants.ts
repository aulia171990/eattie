...
async function requireOwner() {
  ...
  if (!user) return { supabase, isOwner: false }
  ...
  return { supabase, isOwner: profile?.role === 'owner' }
}
...
export async function createVariant(
  ...
  const { supabase, isOwner } = await requireOwner()
  if (!isOwner) return { error: 'Hanya owner yang dapat mengubah varian produk' }
...
export async function deleteVariant(variantId: string, productId: string): Promise<ActionState> {
  const { supabase, isOwner } = await requireOwner()
  if (!isOwner) return { error: 'Hanya owner yang dapat menghapus varian produk' }

  // Cek dulu apakah variant ini pernah dipakai di transaksi — kalau
  // sudah pernah dipakai, JANGAN hapus (akan merusak histori order/sale
  // lama yang referensi ke variant_id ini). Nonaktifkan saja (is_active).
...
export async function createAddon(
  ...
  const { supabase, isOwner } = await requireOwner()
  if (!isOwner) return { error: 'Hanya owner yang dapat mengubah add-on produk' }
...
export async function deleteAddon(addonId: string, productId: string): Promise<ActionState> {
  const { supabase, isOwner } = await requireOwner()
  if (!isOwner) return { error: 'Hanya owner yang dapat menghapus add-on produk' }
