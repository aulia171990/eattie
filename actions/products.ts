...
export async function getProducts(activeOnly = false): Promise<Product[]> {
  ...
  let query = supabase.from('products').select('*, product_variants(stock)').order('name')
  if (activeOnly) query = query.eq('is_active', true)
  const { data, error } = await query
  if (error) throw new Error(error.message)
  // For variant products, the authoritative stock lives in product_variants.stock
  // (per-variant). Aggregate it so list/POS show the real available stock instead
  // of the parent products.current_stock (which is 0 for variant products).
  // products.current_stock remains the source of truth ONLY for non-variant products.
  return (data ?? []).map((p) => {
    const variants = (p as any).product_variants
    if (Array.isArray(variants) && variants.length > 0) {
      const agg = variants.reduce((s: number, v: any) => s + (Number(v.stock) || 0), 0)
      return { ...p, current_stock: agg }
    }
    return p
  })
}
...
export async function getProduct(id: string): Promise<Product> {
  ...
  const { data, error } = await supabase
    ...
    .single()
  if (error) throw new Error(error.message)
  // Aggregate variant stock for variant products (same rule as getProducts).
  const variants = (data as any)?.product_variants
  if (Array.isArray(variants) && variants.length > 0) {
    const agg = variants.reduce((s: number, v: any) => s + (Number(v.stock) || 0), 0)
    return { ...data, current_stock: agg } as Product
  }
  return data
}
