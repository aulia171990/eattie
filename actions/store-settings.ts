...
export type StoreSettings = {
  ...
  instagram: string
  facebook: string
  color_presets: ColorPreset[] | null
  updated_at: string
  updated_by: string | null
}

/** Satu palet warna yang disimpan owner sebagai preset favorit. */
export type ColorPreset = {
  id: string
  name: string
  colors: Partial<Record<keyof Omit<StoreSettings, 'id' | 'company_name' | 'short_name' | 'tagline' | 'logo_url' | 'logo_icon_url' | 'favicon_url' | 'primary_color_hex' | 'whatsapp' | 'instagram' | 'facebook' | 'updated_at' | 'updated_by' | 'color_presets'>, string>>
}
...
const settingsSchema = z.object({
  ...
  accent_subtle_color: z.string().regex(HSL_REGEX),
...
export async function updateStoreSettings(
  ...
  const rawFields: Record<string, unknown> = {
    ...
    accent_subtle_color: (formData.get('accent_subtle_color') as string) ?? '',
...
/**
 * Simpan / perbarui satu preset warna. `preset.id` kosong = preset baru
 * (generate id otomatis). Mengembalikan daftar preset terbaru agar UI bisa
 * langsung merefresh tanpa reload halaman.
 */
export async function saveColorPreset(
  preset: { id?: string; name: string; colors: Record<string, string> }
  ...
  const existing: ColorPreset[] = Array.isArray(current?.color_presets) ? current.color_presets : []
  const id = preset.id?.trim() || crypto.randomUUID()
  const entry: ColorPreset = { id, name: preset.name.trim(), colors: preset.colors }

  const next = existing.some(p => p.id === id)
    ? existing.map(p => (p.id === id ? entry : p))
    : [...existing, entry]
...
/** Hapus satu preset warna berdasarkan id. */
export async function deleteColorPreset(
  id: string
): Promise<{ presets?: ColorPreset[]; error?: string }> {
