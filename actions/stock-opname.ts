...
export async function createOpname(
  ...
  const { data: opnameNumber, error: onErr } = await (supabase.rpc as any)('next_doc_number', {
    p_prefix: 'OPN',
    p_table: 'stock_opnames',
    p_column: 'opname_number',
  })
  if (onErr || !opnameNumber) return { error: 'Gagal membuat nomor opname' }
