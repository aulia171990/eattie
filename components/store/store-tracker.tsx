          ...
          <div className="bg-white rounded-2xl border p-4 space-y-3" style={{ borderColor: 'hsl(36, 20%, 88%)' }}>
            <div className="flex justify-between items-start">
              ...
              <span className="px-2.5 py-1 rounded-full text-xs font-semibold"
                style={{
                  background: ['PAID','paid'].includes(result.payment_status) ? 'hsl(142, 50%, 90%)' : 'hsl(36, 80%, 90%)',
                  color: ['PAID','paid'].includes(result.payment_status) ? 'hsl(142, 60%, 28%)' : 'hsl(32, 95%, 35%)',
                }}>
          ...
          {['COMPLETED','completed'].includes(result.status) && result.order_items.length > 0 && !reviewDone && (
            <div className="bg-white rounded-2xl border p-4 space-y-3"
              ...
              <form onSubmit={handleReviewSubmit} className="space-y-3">
                ...
                <div>
                  <label className="text-xs font-medium mb-2 block" style={{ color: 'hsl(25, 30%, 35%)' }}>Rating</label>
                  <div className="flex gap-1">
                    {[1,2,3,4,5].map(star => (
                      <button key={star} type="button"
                        ...
                        <Star size={28}
                          fill={(reviewHover || reviewRating) >= star ? 'hsl(36, 90%, 50%)' : 'none'}
