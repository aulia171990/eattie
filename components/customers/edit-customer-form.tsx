...
export function EditCustomerForm({ customer }: { customer: Customer }) {
  ...
  return (
    <div className="bg-white rounded-xl border p-4 space-y-3" style={{ borderColor: 'hsl(36, 20%, 88%)' }}>
      <p className="text-xs font-semibold" style={{ color: 'hsl(25, 15%, 45%)' }}>EDIT PROFIL</p>

      <form action={updateAction} className="space-y-3">
        <div>
          <label className="text-xs font-medium" style={{ color: 'hsl(25, 30%, 30%)' }}>Nama</label>
          <input name="name" defaultValue={customer.name} required
            className="w-full mt-1 px-3 py-2 rounded-lg border text-sm outline-none"
            style={{ borderColor: 'hsl(36, 20%, 85%)' }} />
        </div>
        <div>
          <label className="text-xs font-medium" style={{ color: 'hsl(25, 30%, 30%)' }}>Email</label>
          <input name="email" type="email" defaultValue={customer.email ?? ''}
            className="w-full mt-1 px-3 py-2 rounded-lg border text-sm outline-none"
            style={{ borderColor: 'hsl(36, 20%, 85%)' }} />
        </div>
        <div>
          <label className="text-xs font-medium" style={{ color: 'hsl(25, 30%, 30%)' }}>Alamat</label>
        ...
        {updateState && 'success' in updateState && updateState.success && (
          <p className="text-xs" style={{ color: 'hsl(142, 60%, 35%)' }}>✓ Tersimpan</p>
        )}
