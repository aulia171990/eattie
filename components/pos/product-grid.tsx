      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2.5">
        {products.map(product => {
          const p = product as Product & {
            ...
            variant_stock?: { stock: number | null; is_active: boolean | null }[] | null
          }
          const inCart = items.find(i => i.product.id === p.id)

          // Total stock across active variants (null = no variants / not loaded).
          // After migration 000047 parent stock was moved onto the first variant,
          // so a variant product's real availability lives in product_variants.stock.
          // Use variant stock when variants exist, otherwise fall back to product stock.
          const activeVariants = (p.variant_stock ?? []).filter(v => v.is_active !== false)
          const hasVariants = activeVariants.length > 0
          const variantTotalStock = hasVariants
            ? activeVariants.reduce((s, v) => s + (v.stock ?? 0), 0)
            : 0
          const effectiveStock = hasVariants ? variantTotalStock : (p.current_stock ?? 0)
            ...
            >
              ...
              <div className="p-2 flex flex-col gap-0.5">
                ...
                <p className="text-xs" style={{ color: outOfStock ? 'hsl(var(--danger))' : 'hsl(var(--success))' }}>
                  Stok: {hasVariants ? `${variantTotalStock} (varian)` : p.current_stock}
                </p>
              </div>
            </button>
          )
