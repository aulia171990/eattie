...
function ProductTile({ product }: { product: ProductCardType }) {
  ...
  // products.cost_price is schema-nullable (no NULL rows currently in DB); guard with ?? 0 for margin calc
  const margin = (p.cost_price ?? 0) > 0 ? (((p.selling_price ?? 0) - (p.cost_price ?? 0)) / (p.selling_price ?? 0) * 100) : null
      ...
      <div className="p-3">
        ...
        <div className="mt-2 pt-2 border-t" style={{ borderColor: 'hsl(var(--border))' }}>
          ...
          <div className="flex items-center justify-between mt-0.5">
            <p className="text-xs" style={{ color: 'hsl(var(--text-muted))' }}>HPP: {formatCurrency(p.cost_price ?? 0)}</p>
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs" style={{ color: 'hsl(var(--text-muted))' }}>
          HPP: {formatCurrency(p.cost_price ?? 0)}
