    ...
    <form action={formAction} className="space-y-6 max-w-2xl">
      ...
      <div className="bg-white rounded-xl border p-6 space-y-4" style={{ borderColor: 'hsl(var(--border))' }}>
        ...
        <div className='grid grid-cols-2 sm:grid-cols-4 gap-4'>
          <div>
            <label className='text-xs font-medium block mb-1' style={{ color: 'hsl(var(--text-secondary))' }}>Stok Saat Ini</label>
            <p className='px-3 py-2 rounded-lg border text-sm bg-gray-50' style={{ borderColor: 'hsl(var(--border))', color: 'hsl(var(--text-muted))' }}>
              {formatNumber(ingredient?.current_stock ?? 0, 2)} {ingredient?.base_unit ?? ''}
            </p>
            <p className='text-[11px] mt-1' style={{ color: 'hsl(var(--text-muted))' }}>
              Stok diatur otomatis lewat Pembelian / Opname.
            </p>
          </div>
      ...
      <div className="bg-white rounded-xl border p-6 space-y-4" style={{ borderColor: 'hsl(var(--border))' }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Field label="Harga per 1 Satuan Dasar (Rp)" name="price_per_unit" type="number"
            defaultValue={String(ingredient?.price_per_unit ?? 0)} min="0" />
          {ingredient?.base_unit && (
            <p className="text-[11px] -mt-3" style={{ color: 'hsl(var(--text-muted))' }}>
              Harga per 1 {ingredient.base_unit}. Jika membeli per &quot;{ingredient.purchase_unit ?? ingredient.base_unit}&quot;,
              isi harga yang SUDAH dibagi konversi (bukan harga per {ingredient.purchase_unit ?? ingredient.base_unit}).
            </p>
          )}
