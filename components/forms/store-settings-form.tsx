...
import { updateStoreSettings, uploadStoreLogo, saveColorPreset, deleteColorPreset, type StoreSettings, type ColorPreset } from '@/actions/store-settings'
import { previewCssVariables } from '@/contexts/branding-context'
import { Upload, Check, Loader, Palette, Sparkles } from 'lucide-react'
...
/**
 * Dari hue + saturasi, cari lightness (0..100) tertinggi yang masih
 * memenuhi rasio kontras minimal terhadap `fg`. Dipakai untuk mencari
 * teks terang yang kontrasnya cukup di atas warna utama, dan sebaliknya
 * (lightness terendah untuk teks gelap di atas latar terang).
 */
function bestLightnessForContrast(
  h: number,
  s: number,
  fg: string,
  minRatio: number,
  ...
  let best = direction === 'lightest' ? 100 : 0
  let found = false
  const step = 1
  if (direction === 'lightest') {
    for (let l = 100; l >= 0; l -= step) {
      const ratio = contrastRatio(`${h} ${s}% ${l}%`, fg)
      if (ratio >= minRatio) { best = l; found = true; break }
    }
  } else {
  ...
  // Kalau mentok (mis. saturasi terlalu tinggi), pakai ekstrem aman.
  if (!found) best = direction === 'lightest' ? 100 : 0
  return best
}

/** Rotasi hue ±delta, dipakai untuk variasi aksen/footer yang selaras. */
function rotateHue(h: number, delta: number): number {
  return ((h + delta) % 360 + 360) % 360
}

/**
 * Generate palet rekomendasi yang selaras dengan `primaryHsl` dan memenuhi
 * kontras WCAG AA (4.5:1) untuk pasangan yang relevan. Mengembalikan
 * subset field yang bisa langsung diterapkan via handlePick.
 */
function buildRecommendedPalette(primaryHsl: string): Record<string, string> {
  const p = parseHsl(primaryHsl)
  if (!p) return {}
  const { h, s } = p

  // Hero: teks putih di atas primary. Cari teks terang kalau primary-nya
  // terlalu terang, atau biarkan putih kalau sudah cukup.
  const whiteOnPrimary = contrastRatio(primaryHsl, '0 0% 100%')
  const accentFg =
    whiteOnPrimary >= 4.5 ? '0 0% 100%' : `0 0% ${bestLightnessForContrast(h, s, primaryHsl, 4.5, 'lightest')}%`

  // Teks utama di atas surface putih → harus gelap & kontras cukup.
  const textOnSurface = `0 0% ${bestLightnessForContrast(h, 0, '0 0% 100%', 4.5, 'darkest')}%`
  const textMuted = `25 30% ${Math.min(45, bestLightnessForContrast(h, 30, '0 0% 100%', 4.5, 'darkest'))}%`

  // Aksen = hue primary digeser sedikit, sedikit lebih gelap biar kaya.
  const accent = `${rotateHue(h, -12)} ${Math.min(90, s + 5)}% ${Math.max(30, Math.min(50, 100 - bestLightnessForContrast(rotateHue(h, -12), s, '0 0% 100%', 3, 'darkest')))}%`

  // Footer = hue primary gelap (turun ~18 lightness dari primary, minimal 12).
  const footerBgL = Math.max(12, (p.l > 24 ? p.l - 14 : p.l) - 4)
  const footerBg = `${rotateHue(h, -6)} ${Math.max(20, s - 15)}% ${footerBgL}%`
  const footerText = `35 20% ${bestLightnessForContrast(rotateHue(h, -6), Math.max(20, s - 15), footerBg, 4.5, 'lightest')}%`

  // Latar & surface netral hangat biar selaras tapi tetap terang.
  const background = `35 30% 96%`
  // Border = hue primary sangat pucat.
  const border = `${h} ${Math.max(10, s - 60)}% 85%`

  return {
    primary_color: primaryHsl,
    accent_color: accent,
    button_text_color: accentFg,
    accent_foreground_color: accentFg,
    text_color: textOnSurface,
    text_muted_color: textMuted,
    text_secondary_color: textMuted,
...
/** Badge label rasio kontras (reuse logika ContrastRow secara ringkas). */
function contrastBadge(bg: string, fg: string) {
  const ratio = contrastRatio(bg, fg)
  const pass = ratio >= 4.5
  const largePass = ratio >= 3
  return {
    ratio,
    label: pass ? 'AA' : largePass ? 'AA·' : '✗',
    bg: pass ? '#15803d' : largePass ? '#b45309' : '#b91c1c',
  }
}
...
const FIELD_MAP: Record<string, string> = {
  ...
  accent_subtle_color: 'accentSubtleColor',
...
const COLOR_SECTIONS: { title: string; slots: ColorSlot[] }[] = [
  ...
  },
  {
    ...
    slots: [
      ...
      { key: 'accent_subtle_color', label: 'Latar Aksen Halus (badge/section)', options: SURFACE_OPTIONS },
...
/**
 * Peringatan kontras untuk hero store-landing: teks sekunder (`text-muted`)
 * ditampilkan di atas latar `--primary` (hero). Kalau kombinasi dua warna
 * ini gagal AA (4.5:1), owner diperingatkan supaya bisa pilih teks/aksen
 * yang lebih terbaca sebelum menyimpan.
 */
function HeroContrastWarning({ primary, textMuted }: { primary: string; textMuted: string }) {
  const ratio = contrastRatio(primary, textMuted)
  const pass = ratio >= 4.5
  if (pass) return null
  return (
    <div
      className="rounded-2xl border p-4 flex items-start gap-3"
      style={{ borderColor: 'hsl(var(--warning) / 0.5)', background: 'hsl(var(--warning-bg))' }}
      role="alert"
    >
      <span
      ...
      <div className="space-y-1">
        <p className="text-sm font-semibold" style={{ color: 'hsl(var(--warning))' }}>
          Kontras hero landing belum memenuhi AA
        </p>
        <p className="text-xs leading-relaxed" style={{ color: 'hsl(var(--text-secondary))' }}>
          Teks sekunder di atas warna hero (Primary vs Teks Sekunder) hanya {ratio.toFixed(2)}:1.
          Standar WCAG AA butuh minimal 4.5:1. Pilih <strong>Warna Tombol &amp; Hero</strong> yang lebih
          gelap atau <strong>Warna Teks Sekunder</strong> yang lebih terang supaya landing mudah dibaca.
        </p>
      </div>
    </div>
  )
...
/**
 * Panel rekomendasi palet: dari Warna Utama yang dipilih, hitung palet
 * pendukung yang selaras & memenuhi kontras WCAG AA, tampilkan preview
 * mini store-landing supaya mudah dicontoh, lalu bisa diterapkan satu per
 * satu atau sekaligus.
 */
function RecommendedPalettePanel({
  primaryHsl,
  onPick,
  presets,
  onSavePreset,
  ...
  primaryHsl: string
  onPick: (fieldKey: string, hsl: string) => void
  presets: ColorPreset[]
  ...
  const recommended = useMemo(() => buildRecommendedPalette(primaryHsl), [primaryHsl])
  ...
  async function handleSave() {
    if (!presetName.trim()) { setPresetErr('Kasih nama preset dulu'); return }
  ...
  const primary = primaryHsl
  const accentFg = recommended.button_text_color || '0 0% 100%'
  const textOnSurface = recommended.text_color || '25 30% 18%'
  const footerBg = recommended.footer_bg_color || '18 32% 18%'
  const footerText = recommended.footer_text_color || '35 20% 90%'
  const surface = recommended.surface_color || '0 0% 100%'
  const border = recommended.border_color || '18 20% 85%'

  const heroBadge = contrastBadge(primary, accentFg)
  const textBadge = contrastBadge(surface, textOnSurface)
  const footerBadge = contrastBadge(footerBg, footerText)

  const rows: { key: string; label: string; bg?: string; fg?: string }[] = [
    { key: 'accent_color', label: 'Aksen' },
    { key: 'text_color', label: 'Teks Utama' },
    { key: 'text_muted_color', label: 'Teks Sekunder' },
    { key: 'background_color', label: 'Latar' },
    { key: 'surface_color', label: 'Surface' },
    { key: 'border_color', label: 'Garis' },
    { key: 'footer_bg_color', label: 'Footer' },
    { key: 'footer_text_color', label: 'Teks Footer' },
  ]
    ...
    <div className="bg-white rounded-2xl border p-5 space-y-4" style={{ borderColor: 'hsl(var(--border))' }}>
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-sm font-bold" style={{ color: 'hsl(var(--foreground))' }}>
          Rekomendasi Palet (otomatis AA)
        </h3>
      </div>
      <p className="text-xs leading-relaxed" style={{ color: 'hsl(var(--text-secondary))' }}>
        Dari <strong>Warna Utama</strong> yang dipilih, kami hitung palet pendukung yang selaras &amp;
        memenuhi kontras WCAG AA (4.5:1). Lihat preview-nya, lalu terapkan satu per satu atau sekaligus.
      </p>

      {/* Preview mini store-landing */}
      <div className="rounded-xl overflow-hidden border" style={{ borderColor: border ? `hsl(${border})` : 'hsl(var(--border))' }}>
        {/* Hero */}
        <div className="px-4 py-5" style={{ background: `hsl(${primary})` }}>
          <p className="text-[15px] font-extrabold leading-tight" style={{ color: `hsl(${accentFg})` }}>
            Roti Segar Harian
          </p>
          <p className="text-[11px] mt-0.5" style={{ color: `hsl(${accentFg})`, opacity: 0.9 }}>
            Panggang setiap pagi untukmu
          </p>
          <span
            className="inline-block mt-2 px-3 py-1 rounded-full text-[11px] font-bold"
            style={{ background: `hsl(${accentFg})`, color: `hsl(${primary})` }}
          >
            Pesan Sekarang
          </span>
        </div>
        {/* Product cards */}
        <div className="px-4 py-3 grid grid-cols-2 gap-2" style={{ background: `hsl(${recommended.background_color || '35 30% 96%'})` }}>
          {[1, 2].map(n => (
            <div key={n} className="rounded-lg p-2 border" style={{ background: `hsl(${surface})`, borderColor: `hsl(${border})` }}>
              <div className="h-8 rounded-md mb-1.5" style={{ background: `hsl(${primary})`, opacity: 0.18 }} />
              <p className="text-[10px] font-semibold" style={{ color: `hsl(${textOnSurface})` }}>Roti {n}</p>
              <p className="text-[9px]" style={{ color: `hsl(${textOnSurface})`, opacity: 0.7 }}>Rp 25.000</p>
            </div>
          ))}
        </div>
        {/* Footer */}
        <div className="px-4 py-3" style={{ background: `hsl(${footerBg})` }}>
          <p className="text-[10px]" style={{ color: `hsl(${footerText})` }}>
            © 2026 Toko Roti · Follow kami
          </p>
        </div>
      </div>

      {/* Ringkasan kontras kunci */}
      <div className="space-y-1 text-[10px] font-mono">
        <div className="flex items-center justify-between">
          <span style={{ color: 'hsl(var(--text-muted))' }}>Teks tombol vs Hero</span>
          <span className="flex items-center gap-1">
            <span style={{ color: 'hsl(var(--text-muted))' }}>{heroBadge.ratio.toFixed(2)}:1</span>
            <span className="px-1 rounded text-white" style={{ background: heroBadge.bg }}>{heroBadge.label}</span>
          </span>
        </div>
        <div className="flex items-center justify-between">
          <span style={{ color: 'hsl(var(--text-muted))' }}>Teks vs Surface</span>
          <span className="flex items-center gap-1">
            <span style={{ color: 'hsl(var(--text-muted))' }}>{textBadge.ratio.toFixed(2)}:1</span>
            <span className="px-1 rounded text-white" style={{ background: textBadge.bg }}>{textBadge.label}</span>
          </span>
        </div>
        <div className="flex items-center justify-between">
          <span style={{ color: 'hsl(var(--text-muted))' }}>Teks vs Footer</span>
          <span className="flex items-center gap-1">
            <span style={{ color: 'hsl(var(--text-muted))' }}>{footerBadge.ratio.toFixed(2)}:1</span>
            <span className="px-1 rounded text-white" style={{ background: footerBadge.bg }}>{footerBadge.label}</span>
          </span>
        </div>
      </div>

      {/* Daftar swatch rekomendasi */}
      <div className="grid grid-cols-2 gap-2">
        {rows.map(r => {
          const hsl = recommended[r.key]
          if (!hsl) return null
          return (
            <button
              key={r.key}
              type="button"
              onClick={() => onPick(r.key, hsl)}
              className="flex items-center gap-2 pl-1.5 pr-2 py-1.5 rounded-full border-2 transition-all text-[11px] font-medium"
              style={{ borderColor: 'hsl(var(--border))', background: 'white', color: 'hsl(var(--text-secondary))' }}
              title={`Terapkan ${r.label}`}
            >
              <span
      ...
      <button
        type="button"
        onClick={() => {
          for (const [key, hsl] of Object.entries(recommended)) onPick(key, hsl)
        }}
        className="w-full py-2.5 rounded-xl text-xs font-bold text-white flex items-center justify-center gap-1.5"
        style={{ background: `hsl(${primary})`, color: `hsl(${accentFg})` }}
      >
        <Sparkles size={13} /> Terapkan Semua Rekomendasi
      </button>

      {/* Simpan palet saat ini sebagai preset */}
      <div className="pt-3 border-t space-y-2" style={{ borderColor: 'hsl(var(--border))' }}>
        <p className="text-xs font-semibold" style={{ color: 'hsl(var(--text-secondary))' }}>
          Simpan palet ini sebagai preset
        </p>
        <div className="flex gap-2">
          <input
            ...
            placeholder="Nama preset (mis. Cokelat Hangat)"
          ...
          <button
            ...
            className="px-3 py-2 rounded-lg text-xs font-bold text-white flex items-center gap-1 disabled:opacity-60"
            style={{ background: `hsl(${primary})`, color: `hsl(${accentFg})` }}
          >
            {savingPreset ? <Loader size={13} className="animate-spin" /> : <Check size={13} />}
            Simpan
          </button>
        </div>
        {presetErr && (
          <p className="text-[11px]" style={{ color: 'hsl(var(--danger))' }}>{presetErr}</p>
        )}
      </div>
      ...
      {presets.length > 0 && (
        <div className="space-y-2">
          <p className="text-xs font-semibold" style={{ color: 'hsl(var(--text-secondary))' }}>
            Preset Tersimpan ({presets.length})
          </p>
          <div className="space-y-2">
            {presets.map(p => {
              const swatches = Object.values(p.colors).slice(0, 6)
                  ...
                  <div className="flex -space-x-1.5">
                    {swatches.map((c, i) => (
                      <span
                        ...
                        style={{ background: `hsl(${c})`, borderColor: 'white' }}
                      />
                    ))}
                    {swatches.length === 0 && (
                      <span className="text-[10px]" style={{ color: 'hsl(var(--text-muted))' }}>—</span>
                    )}
                  </div>
                  <span className="flex-1 text-xs font-medium truncate" style={{ color: 'hsl(var(--text-secondary))' }}>
                    {p.name}
                  </span>
                  <button
                    type="button"
                    ...
                    title="Terapkan preset ke form"
                  >
                    {isApplying ? <Loader size={12} className="animate-spin" /> : 'Pakai'}
                  </button>
                  <button
                    type="button"
                    onClick={() => onDeletePreset(p.id)}
                    className="px-2 py-1 rounded-md text-[11px] font-semibold border"
                    style={{ borderColor: 'hsl(var(--danger) / 0.4)', color: 'hsl(var(--danger))' }}
                    title="Hapus preset"
                  >
                    Hapus
                  </button>
...
export function StoreSettingsForm({ settings }: Props) {
  ...
  const [presets, setPresets] = useState<ColorPreset[]>(settings?.color_presets ?? [])
  ...
  async function handleSavePreset(name: string) {
    ...
    for (const key of Object.keys(FIELD_MAP)) {
      if (colors[key]) colorsToSave[key] = colors[key]
    }
    const res = await saveColorPreset({ name, colors: colorsToSave })
    if (res.error) { setPresetMsg(res.error); return }
    if (res.presets) setPresets(res.presets)
    setPresetMsg('Preset tersimpan')
    setTimeout(() => setPresetMsg(null), 2500)
  }

  async function handleDeletePreset(id: string) {
    const res = await deleteColorPreset(id)
    if (res.error) { setPresetMsg(res.error); return }
    if (res.presets) setPresets(res.presets)
  }

  function applyPreset(p: ColorPreset) {
    setApplyingPreset(p.id)
    for (const [k, v] of Object.entries(p.colors)) {
      if (FIELD_MAP[k]) handlePick(k, v)
    }
    setTimeout(() => setApplyingPreset(null), 300)
  }
    ...
    <div className="space-y-6 max-w-3xl">
      ...
      <form action={formAction} className="space-y-6">
        ...
        <HeroContrastWarning
          ...
          textMuted={colors.text_muted_color}
        />

        <RecommendedPalettePanel
          primaryHsl={colors.primary_color}
          onPick={handlePick}
          presets={presets}
        ...
        {presetMsg && (
          <p className="text-xs px-4 py-2 rounded-lg" style={{ background: 'hsl(var(--success-bg))', color: 'hsl(var(--success))' }}>
            {presetMsg}
          </p>
        )}
