...
export function convertQRIS(
  ...
  for (const el of elements) {
    ...
    // Insert amount + fee before tag 58 (Country Code) when present.
  ...
  // Fallback: some static QRIS strings omit the country code (tag 58), so the
  // amount was never inserted above. Append it at the end before the CRC.
  if (!amountInserted) {
