...
| 10 | Tabel `customers` sama sekali tidak punya RLS — terbuka penuh di level database | ✅ **SUDAH DIPERBAIKI & TERVERIFIKASI** | Database — RLS `customers` | RLS sudah aktif dengan 5 policy (select/insert/update/delete + satu `FOR ALL` yang redundan tapi tidak berbahaya). Semua kondisi konsisten: `get_user_role() = 'owner'` — tidak ada yang lebih longgar dari yang lain (beda dengan Bug 7). Dikerjakan langsung di Supabase SQL Editor, **belum tercatat sebagai file migration di repo** — kalau project Supabase pernah rebuild, policy ini bisa hilang tanpa jejak (pola sama seperti Bug 4 sebelum diperbaiki) |
...
| 12 | Review produk dari customer tidak pernah tersimpan — RLS tolak diam-diam | 🔴 Tinggi — **PATCH SUDAH DIBUAT, PERLU DIJALANKAN** | Database RLS `product_reviews`, migration `000001_product_reviews_featured.sql` | Customer isi review di `/store/track` **tanpa login** (role `anon`), tapi policy INSERT yang ada (`product_reviews_staff_insert`) cuma untuk `authenticated`. Review selalu ditolak RLS secara diam-diam — tidak ada error yang terlihat customer, sistem terkesan "berhasil kirim" tapi data tidak pernah masuk database. Validasi kepemilikan (order COMPLETED, product cocok, phone cocok) sudah benar dilakukan di `app/api/reviews/route.ts` sebelum insert, jadi fix cukup di level RLS. File fix: `supabase/migrations/000003_product_reviews_anon_insert.sql` — **wajib dijalankan di Supabase SQL Editor** |
